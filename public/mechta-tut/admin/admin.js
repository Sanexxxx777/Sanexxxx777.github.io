...
function markDirty(){if(isDirty)return;isDirty=true;dirtyIndicator.hidden=false}
function clearDirty(){isDirty=false;dirtyIndicator.hidden=true}
function formatThousands(value){var n=Math.round(Number(value)||0);var sign=n<0?"-":"";var s=Math.abs(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g," ");return sign+s}
function updateHallPriceHint(input){var hint=input.nextElementSibling;if(hint)hint.textContent="На сайте: от "+formatThousands(input.value)+" ₽/час"}
function updateHallCapacityHint(input){var hint=input.nextElementSibling;if(hint)hint.textContent="На сайте: до "+formatThousands(input.value)+" человек"}
function updatePricePerMinuteHint(){var hint=document.getElementById("pricePerMinuteHint");if(hint)hint.textContent="На сайте: "+formatThousands(document.getElementById("pricePerMinute").value)+" ₽ в минуту"}
function updateDayCapHint(){var hint=document.getElementById("dayCapHint");if(hint)hint.textContent="На сайте: "+formatThousands(document.getElementById("dayCap").value)+" ₽ максимум за день"}
...
function applyContent(data){renderHalls(data.halls);document.getElementById("pricePerMinute").value=Number(data.free.pricePerMinute);document.getElementById("dayCap").value=Number(data.free.dayCap);updatePricePerMinuteHint();updateDayCapHint();renderAnnouncements(data.announcements)}
function loadContent(){setStatus(contentStatus,"Загружаем…","");request("content").then(function(result){if(result.status===401){showLogin();return}if(!result.ok)throw new Error("load");applyContent(result.body.content);clearDirty();setStatus(contentStatus,"","")}).catch(function(){setStatus(contentStatus,"Не удалось загрузить данные.","err")})}
...
document.getElementById("addAnnounceBtn").addEventListener("click",function(){if(announceList.children.length>=20){setStatus(contentStatus,"Можно добавить не больше 20 анонсов.","err");return}var card=announcementCard({date:"",title:"",text:""});announceList.appendChild(card);card.querySelector("input").focus();markDirty()});
document.getElementById("saveBtn").addEventListener("click",function(){var firstInvalid=document.querySelector("#adminApp input:invalid,#adminApp textarea:invalid");if(firstInvalid){firstInvalid.reportValidity();firstInvalid.focus();return}var button=this;busy(button,true);setStatus(contentStatus,"Сохраняем…","");request("content",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":csrfToken},body:JSON.stringify({content:collectContent()})}).then(function(result){busy(button,false);if(result.status===401){showLogin();return}if(result.status===403&&result.body.error){setStatus(contentStatus,result.body.error,"err");return}if(result.ok){clearDirty();setStatus(contentStatus,"Сохранено. Изменения уже доступны сайту.","ok")}else setStatus(contentStatus,result.body.error||"Не удалось сохранить.","err")}).catch(function(){busy(button,false);setStatus(contentStatus,"Нет соединения с сервером.","err")})});

document.getElementById("adminApp").addEventListener("input",function(event){var target=event.target;if(target.classList.contains("hall-price"))updateHallPriceHint(target);else if(target.classList.contains("hall-capacity"))updateHallCapacityHint(target);else if(target.id==="pricePerMinute")updatePricePerMinuteHint();else if(target.id==="dayCap")updateDayCapHint();markDirty()});
window.addEventListener("beforeunload",function(event){if(!isDirty)return;event.preventDefault();event.returnValue=""});
if(savebar&&window.ResizeObserver){new ResizeObserver(function(entries){var height=entries[0].borderBoxSize?entries[0].borderBoxSize[0].blockSize:entries[0].contentRect.height;document.documentElement.style.setProperty("--savebar-h",Math.ceil(height)+"px")}).observe(savebar)}
