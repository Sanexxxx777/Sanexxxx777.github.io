...
export function kineticElement(el: HTMLElement): () => void {
  ...
  const PUSH = 9;    // макс. горизонтальный уход ≈ 0.6·PUSH px (пик профиля на dx = σ)
  let raf = 0;
  let px = -1e4, py = -1e4;
  const frame = () => {
    raf = 0;
    ...
    letters.forEach((l, i) => {
      ...
      // буква уходит от курсора: по X профиль «производная гауссианы» (под курсором 0,
      // пик на расстоянии σ — нет скачка направления при проходе через центр), по Y — приподнимается
      const tx = -(dx / SIGMA) * f * PUSH;
      const ty = -4 * f;
      l.style.transform = f > 0.02 ? `translate(${tx.toFixed(2)}px, ${ty.toFixed(2)}px)` : "";
    });
  };
  let inZone = false;
